...
void main() async {
  ...
  final service = IntegraContadorBuilder()
      ...
      .withProcuradorToken('TOKEN_PROCURADOR_OPCIONAL')
...
Future<void> exemploSimplesnacional(IntegraContadorExtendedService service) async {
  ...
  final declaracoes = await service.consultarDeclaracoesSN(
    documento: cnpj,
    anoCalendario: '2024',
  );
  ...
  final das = await service.gerarDASSN(
    documento: cnpj,
  ...
  final extrato = await service.consultarExtratoDAS(
    documento: cnpj,
    periodoApuracao: periodo,
  );
  ...
  final regime = await service.consultarOpcaoRegimeApuracao(
    documento: cnpj,
    anoCalendario: '2024',
  );
...
Future<void> exemploMEI(IntegraContadorExtendedService service) async {
  ...
  final dasPDF = await service.gerarDASMEIPDF(
    documento: cpf,
    periodoApuracao: periodo,
  );
  ...
  final certificado = await service.emitirCertificadoCondicaoMEI(
    cpf: cpf,
  );
  ...
  final situacao = await service.consultarSituacaoCadastralMEI(
    cpf: cpf,
  );
  ...
  final dividaAtiva = await service.consultarDividaAtivaMEI(
    documento: cpf,
  );
...
Future<void> exemploDEFIS(IntegraContadorExtendedService service) async {
  ...
  final declaracoes = await service.consultarDeclaracoesDEFIS(
    documento: cnpj,
    anoCalendario: '2024',
  );
  ...
  final ultima = await service.consultarUltimaDeclaracaoDEFIS(
    documento: cnpj,
    anoCalendario: '2024',
  );
...
Future<void> exemploDCTFWeb(IntegraContadorExtendedService service) async {
  ...
  final completa = await service.consultarDeclaracaoCompletaDCTF(
    documento: cnpj,
    periodoApuracao: periodo,
  );
  ...
  final guia = await service.gerarGuiaDeclaracaoDCTF(
    documento: cnpj,
    periodoApuracao: periodo,
    parametrosAdicionais: {
      'tipo_declaracao': 'normal',
...
Future<void> exemploOutrosSistemas(IntegraContadorExtendedService service) async {
  ...
  final sitfis = await service.consultarSituacaoFiscalSITFIS(
    documento: cnpj,
    anoBase: '2024',
  );
  ...
  final debitos = await service.consultarDebitos(
    documento: cnpj,
    situacao: 'pendente',
  );
  ...
  final parcelamentos = await service.consultarParcelamentos(
    documento: cnpj,
    situacao: 'ativo',
  );
  ...
  final mensagens = await service.consultarMensagensContribuinte(
    documento: cnpj,
  ...
  final darf = await service.consolidarGerarDARFPDF(
    documento: cnpj,
    dadosCalculo: {
      'codigo_receita': '0220',
      'periodo_apuracao': '012024',
  ...
  final procuracao = await service.obterProcuracao(
    documento: cnpj,
    tipoProcuracao: 'eletronica',
  );
...
Future<void> exemploMetodosConveniencia(IntegraContadorExtendedService service) async {
  ...
  final informacoes = await service.consultarInformacoesCompletas(
    documento: cnpj,
    anoBase: '2024',
  );
  ...
  final todosDASCNPJ = await service.gerarTodosDAS(
    documento: cnpj,
  ...
  final todosDASCPF = await service.gerarTodosDAS(
    documento: cpf,
    periodoApuracao: '012024',
  );
  ...
  final declaracoes = await service.consultarTodasDeclaracoes(
    documento: cnpj,
    anoCalendario: '2024',
  );
...
Future<void> exemploTratamentoErros() async {
  final service = IntegraContadorBuilder()
      .withJwtToken('token_invalido')
      .build();
  
  print('\n🛡️ === EXEMPLO DE TRATAMENTO DE ERROS ===');
  
  final result = await service.consultarSituacaoFiscalSITFIS(
...
Future<void> exemploComCache() async {
  ...
  // Criar serviço base
  final baseService = IntegraContadorBuilder()
      .withJwtToken('SEU_TOKEN_JWT_AQUI')
      .buildBaseService();
  
  // Envolver com cache
  final cachedService = CachedIntegraContadorService(
    baseService,
  ...
  final result1 = await cachedService.consultarSituacaoFiscalSITFIS(
    documento: documento,
    anoBase: '2024',
  );
  ...
  final result2 = await cachedService.consultarSituacaoFiscalSITFIS(
    documento: documento,
    anoBase: '2024',
  );
...
class CachedIntegraContadorService extends IntegraContadorExtendedService {
  ...
  Future<ApiResult<DadosSaida>> consultarSituacaoFiscalSITFIS({
    ...
    final result = await super.consultarSituacaoFiscalSITFIS(
      documento: documento,
      anoBase: anoBase,
    );
