...
class PgdasdDominios {
  ...
  // Tipos de declaração
  static const Map<String, String> tipoDeclaracao = {
    '1': 'Original',
    '2': 'Retificadora',
  };
  ...
  static const Map<String, String> idAtividade = {
    ...
    '12':
        'Prestação de serviços, exceto para o exterior - Sujeitos ao fator “r”, com retenção/substituição tributária de ISS',
    '13':
    ...
    '18':
        'Prestação de serviços, exceto para o exterior - Sujeitos ao Anexo IV, com retenção/substituição tributária de ISS',
    '19':
    ...
    '30':
        'Prestação de serviços para o exterior - Não sujeitos ao fator “r” e tributados pelo Anexo III',
    ...
    '42':
        'Atividades com incidência simultânea de IPI e de ISS, exceto para o exterior - Com retenção/substituição tributária de ISS',
    '43':
        'Atividades com incidência simultânea de IPI e de ISS para o exterior',
  };

  // Tipo de Redução
  static const Map<String, String> tipoReducao = {
    '1': 'Normal',
    '2': 'Cesta básica',
  };
  ...
  // Helpers
  static String descricaoTipoDeclaracao(dynamic codigo) =>
      tipoDeclaracao[codigo?.toString() ?? ''] ?? (codigo?.toString() ?? '');
  static String descricaoCodigoTributo(dynamic codigo) =>
      codigoTributo[codigo?.toString() ?? ''] ?? (codigo?.toString() ?? '');
  static String descricaoAtividade(dynamic id) =>
      idAtividade[id?.toString() ?? ''] ?? (id?.toString() ?? '');
  static String descricaoTipoReducao(dynamic codigo) =>
      tipoReducao[codigo?.toString() ?? ''] ?? (codigo?.toString() ?? '');
  static String descricaoTipoIsencao(dynamic codigo) =>
      tipoIsencao[codigo?.toString() ?? ''] ?? (codigo?.toString() ?? '');
  static String descricaoMotivoExigibilidade(dynamic codigo) =>
      motivoExigibilidadeSuspensa[codigo?.toString() ?? ''] ??
      (codigo?.toString() ?? '');
  static String descricaoProrrogacaoEspecial(dynamic codigo) =>
      prorrogacaoEspecial[codigo?.toString() ?? ''] ??
      (codigo?.toString() ?? '');
}
