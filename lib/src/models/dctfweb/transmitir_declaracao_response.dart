...
class TransmitirDeclaracaoDctfResponse {
  ...
  TransmitirDeclaracaoDctfResponse({required this.status, required this.dados, required this.mensagens});
  ...
  bool get temMaed =>
      mensagens.any((m) => m.codigo == 'TRANS14' || m.texto.toLowerCase().contains('maed') || m.texto.toLowerCase().contains('multa por atraso'));
...
class TransmissaoInfo {
  ...
  TransmissaoInfo({this.numeroRecibo, this.dataTransmissao, this.horaTransmissao, this.situacao});
  ...
  Map<String, dynamic> toJson() {
    return {'numeroRecibo': numeroRecibo, 'dataTransmissao': dataTransmissao, 'horaTransmissao': horaTransmissao, 'situacao': situacao};
  }
}
