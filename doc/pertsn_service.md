...
O `PertsnService` é responsável pela integração com o sistema PERTSN (Parcelamento do Simples Nacional). Este serviço permite consultar pedidos de parcelamento, consultar parcelamentos específicos, consultar parcelas disponíveis para impressão, consultar detalhes de pagamento e emitir DAS para parcelas.

## Funcionalidades Principais

- **Consultar Pedidos**: Consulta todos os pedidos de parcelamento PERTSN
...
void main() async {
  ...
  try {
    // 1. Consultar pedidos
    print('=== Consultando Pedidos PERTSN ===');
    ...
    if (pedidos.sucesso) {
      ...
      for (final parcelamento in parcelamentos) {
        ...
        final detalhes = await pertsnService.consultarParcelamento(parcelamento.numero);
        if (detalhes.sucesso) {
    ...
    final parcelas = await pertsnService.consultarParcelas();
    ...
    if (parcelas.dadosParsed?.listaParcelas.isNotEmpty == true) {
      ...
      final das = await pertsnService.emitirDas(primeiraParcela.parcelaParaEmitir);
...
Future<void> consultarParcelamentoCompleto(int numeroParcelamento) async {
  try {
    ...
    final parcelamento = await pertsnService.consultarParcelamento(numeroParcelamento);
...
Future<void> emitirDasLote() async {
  try {
    ...
    for (final parcela in listaParcelas) {
      try {
        final das = await pertsnService.emitirDas(parcela.parcelaParaEmitir);
        if (das.sucesso && das.pdfGeradoComSucesso) {
